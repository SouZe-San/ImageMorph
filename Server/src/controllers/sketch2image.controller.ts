...
import {
  ASSETS_FOLDER_FOR_SKETCH_GENERATION,
  STABILITY_ENGINE_ID,
} from "../constant";

const sketch2imageConvert = async (req: Request, res: Response) => {
  ...
  try {
    ...
    const response = await fetch(`${apiHost}/control/sketch`, {
      method: "POST",
      headers: {
    ...
    res.status(resStatus.Created).json(
      new ApiResponse(resStatus.Success, "SketchImage", {
        link: responsePath,
      }),
    );
  } catch (error) {
